	...
	public function trigger_threshold_met_notification_email() {
		...
		if ( $is_notification_enabled && is_email( $admin_email ) && ( $logs_count >= $logs_threshold ) ) {
			...
			$message = <<<EOT
                ...
                <p>Your log threshold of $logs_threshold has been met. You may manually delete the logs to keep your 
                database table in size.</p>

                <p>Also, consider using our <a href="https://wpemaillog.com/addons/auto-delete-logs/">Auto Delete 
                Logs</a> plugin to delete the logs automatically.</p>
EOT;
